package uy.viruscontrol.test.dao;

import static org.junit.Assert.*;

import org.junit.Before;
import org.junit.Test;
import org.mockito.Mock;
import org.mockito.Mockito;
import org.mockito.MockitoAnnotations;

import uy.viruscontrol.model.dao.impl.PlantillaNotificacionDAO;
import uy.viruscontrol.model.entities.PlantillaNotificacion;

public class PlantillaNotificacionDAOTest {

	@Mock
	private PlantillaNotificacionDAO dao;
	
	@Before

    public void setUp() throws Exception {

         MockitoAnnotations.initMocks(this);

    }
	@Test
	public void testPersist() {
		PlantillaNotificacion obj = new PlantillaNotificacion();
		dao.persist(obj);
		Mockito.verify(dao).persist(obj);
	}

	@Test
	public void testMerge() {
		PlantillaNotificacion obj = new PlantillaNotificacion();
		dao.merge(obj);
		Mockito.verify(dao).merge(obj);
	}

	@Test
	public void testFindAll() {
		dao.findAll();
		Mockito.verify(dao).findAll();
	}

	@Test
	public void testFindById() {
		dao.findById(100);
		Mockito.verify(dao).findById(100);
	}

	@Test
	public void testDelete() {
		PlantillaNotificacion obj = new PlantillaNotificacion();
		dao.delete(obj);
		Mockito.verify(dao).delete(obj);
	}

}

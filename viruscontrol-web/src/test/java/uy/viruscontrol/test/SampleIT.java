...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        EnterpriseArchive ear = ShrinkWrap.create(EnterpriseArchive.class, "viruscontrol-ear.ear");

        // Current directory is the root of the "viruscontrol-web" project. Go up one level
        // and enter the "viruscontrol-ejb" project.
        // The ejb jar is found in the "target" directory:
        File f = new File("../viruscontrol-ejb/target/viruscontrol-ejb.jar");
        JavaArchive ejbJar = ShrinkWrap.create(ZipImporter.class, "viruscontrol-ejb.jar").importFrom(f).as(JavaArchive.class);
        ear.addAsModule(ejbJar);

        // Now grab the web archive:
        f = new File("./target/viruscontrol-web.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "viruscontrol-web.war").importFrom(f).as(WebArchive.class);
        ear.addAsModule(war);
        ...
        war.addPackage("uy.viruscontrol.test");
